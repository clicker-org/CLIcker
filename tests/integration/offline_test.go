...
func TestOfflineApply_WorldScreen_EarnsCoins(t *testing.T) {
	...
	result := offline.Apply("world", "terra", savedAt, &eng.State)

	// Expected: 100 CPS * 0.10 * 14400s = 144,000 (well under the 8h cap of 288,000).
	expected := 100.0 * offline.WorldOfflinePct * (4 * 3600)
...
func TestOfflineApply_WorldScreen_CappedAtMaxHours(t *testing.T) {
	...
	expectedMax := 100.0 * offline.WorldOfflinePct * offline.WorldOfflineCapHours * 3600
...
func TestOfflineApply_OverviewScreen_EarnsGeneralCoins(t *testing.T) {
	...
	savedAt := time.Now().Add(-4 * time.Hour)
	result := offline.Apply("overview", "", savedAt, &eng.State)
...
func TestOfflineApply_WithEarnedCPS_CorrectlyAccumulates(t *testing.T) {
	...
	result := offline.Apply(sf.LastScreen, sf.LastWorldID, savedAt, &gs)

	// Expected offline income: cps * 10% * 3600s (under 8h cap for small CPS).
	expected := cps * offline.WorldOfflinePct * 3600
